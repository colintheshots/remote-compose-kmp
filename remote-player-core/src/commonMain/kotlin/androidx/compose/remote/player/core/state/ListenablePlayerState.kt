 ...
 */
class ListenablePlayerState(initialValues: PlayerStateMap = PlayerStateMap.empty()) : PlayerState {
    ...
    private val onStateUpdateCallbacks: MutableList<() -> Unit> = mutableListOf()

    /** A constructor that initializes the player state with the provided values. */
    constructor(init: MapScope.() -> Unit) : this(
        PlayerStateMap(MapScope(mutableMapOf()).apply { init() }.values.toMap())
    )
