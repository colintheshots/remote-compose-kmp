...
interface PlayerState {
     ...
     */
    class MapScope(val values: MutableMap<String, RcValue>) {
        /** Set a value. */
        fun setUserLocalValue(name: String, value: RcValue) {
            values[name] = value
        }
    }
     ...
     */
    class PlayerStateMap(val values: Map<String, RcValue>) {
        companion object {
            fun empty(): PlayerStateMap = PlayerStateMap(emptyMap())
        }
    }
}
...
internal fun PlayerState.applyTo(stateUpdater: StateUpdater) {
    values.forEach { (name, rcValue) ->
        when (rcValue) {
            is RcLong -> stateUpdater.setNamedLong(name, rcValue.value)
            is RcFloat -> stateUpdater.setUserLocalFloat(name, rcValue.value)
            is RcString -> stateUpdater.setUserLocalString(name, rcValue.value)
            is RcInt -> stateUpdater.setUserLocalInt(name, rcValue.value)
            is RcColor -> stateUpdater.setUserLocalColor(name, rcValue.value)
            is RcBitmap -> stateUpdater.setUserLocalBitmap(name, rcValue.value)
        }
    }
}
