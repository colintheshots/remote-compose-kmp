...
/** A [PaintChanges] implementation for Android using framework Paint. */
internal class AndroidComposePaintChanges(
        ...
        when (fontType) {
            ...
            PaintBundle.FONT_TYPE_SANS_SERIF ->
                if (weight == 400 && !italic) {
                    ...
                    getNativePaint().setTypeface(Typeface.create(Typeface.SANS_SERIF, weight, italic))
                }
            PaintBundle.FONT_TYPE_MONOSPACE ->
                if (weight == 400 && !italic) {
                    getNativePaint().setTypeface(Typeface.MONOSPACE)
                } else {
                    getNativePaint().setTypeface(Typeface.create(Typeface.MONOSPACE, weight, italic))
                }
            else -> {
    ...
    private fun setAxis(axis: Array<FontVariationAxis?>?) {
        ...
        val typeface = Typeface.CustomFallbackBuilder(fontFamily).setSystemFallback("sans-serif").build()
        getNativePaint().setTypeface(typeface)
    }

    private fun getFontPath(fontName: String): String? {
        var fontNameLower = fontName.lowercase()
        ...
        for (fontFile in fontFiles) {
            if (fontFile.name.lowercase().contains(fontNameLower)) {
                return fontFile.absolutePath
            }
        }
        Log.i(TAG, "font \"$fontNameLower\" not found")
        return null
    }
    ...
    override fun setTextureShader(
        ...
        val bitmap = remoteContext.mRemoteComposeState.getFromId(bitmapId) as android.graphics.Bitmap? ?: return
        val bs = BitmapShader(
            bitmap,
    ...
    override fun setStrokeCap(cap: Int) {
        getPaint().strokeCap = android.graphics.Paint.Cap.entries[cap].toStrokeCap()
    }

    override fun setStyle(style: Int) {
        getPaint().style = android.graphics.Paint.Style.entries[style].toPaintingStyle()
    }
    ...
    override fun setShader(shaderId: Int) {
        ...
        val shader = RuntimeShader(remoteContext.getText(data.getShaderTextId())!!)
        ...
        for (i in names.indices) {
            val name = names[i]
            val v = data.getUniformInts(name)
            shader.setIntUniform(name, v)
        }
        names = data.getUniformBitmapNames()
        for (i in names.indices) {
            val name = names[i]
            val v = data.getUniformBitmapId(name)
            val bitmap = remoteContext.mRemoteComposeState.getFromId(v) as android.graphics.Bitmap?
    ...
    override fun setAlpha(a: Float) {
        getNativePaint().alpha = (255 * a).toInt()
    }
    ...
    override fun setStrokeJoin(join: Int) {
        getPaint().strokeJoin = android.graphics.Paint.Join.entries[join].toStrokeJoin()
    }
    ...
    override fun setRadialGradient(
        ...
        getNativePaint().setShader(
            RadialGradient(centerX, centerY, radius, colors, stops, tileModes[tileMode])
        )
    }
