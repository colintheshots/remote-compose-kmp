...
open class ColumnLayout(parent: Component?, componentId: Int, animationId: Int, x: Float, y: Float, width: Float, height: Float,
    ...
    override fun toString(): String = "COLUMN [$componentId:$mAnimationId]"
    override fun getSerializedName(): String = "COLUMN"
    ...
    companion object {
        ...
        fun name(): String = "ColumnLayout"; fun id(): Int = Operations.LAYOUT_COLUMN
        fun apply(buffer: WireBuffer, cid: Int, aid: Int, hp: Int, vp: Int, sb: Float) {
            buffer.start(Operations.LAYOUT_COLUMN); buffer.writeInt(cid); buffer.writeInt(aid); buffer.writeInt(hp); buffer.writeInt(vp); buffer.writeFloat(sb) }
        fun read(buffer: WireBuffer, ops: MutableList<Operation>) {
            ops.add(ColumnLayout(null, buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readFloat())) }
    }
}
