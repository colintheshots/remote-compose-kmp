...
class IntFloatMap {
    ...
    init {
        ...
        values = FloatArray(DEFAULT_CAPACITY)
    }

    /** Clear the map */
    fun clear() {
        keys.fill(NOT_PRESENT)
        values.fill(Float.NaN)
        size = 0
    }
    ...
    fun get(key: Int): Float {
        ...
        return if (index == -1) 0f else values[index]
    }

    private fun insert(key: Int, value: Float): Float {
