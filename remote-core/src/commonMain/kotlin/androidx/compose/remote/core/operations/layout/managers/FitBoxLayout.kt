...
open class FitBoxLayout(parent: Component?, componentId: Int, animationId: Int, x: Float, y: Float, width: Float, height: Float,
    ...
    override fun toString(): String = "FITBOX [$componentId:$mAnimationId]"
    override fun getSerializedName(): String = "FITBOX"
    ...
    companion object {
        ...
        fun name(): String = "FitBoxLayout"; fun id(): Int = Operations.LAYOUT_FIT_BOX
        fun apply(buffer: WireBuffer, cid: Int, aid: Int, hp: Int, vp: Int, sb: Float) {
            buffer.start(Operations.LAYOUT_FIT_BOX); buffer.writeInt(cid); buffer.writeInt(aid); buffer.writeInt(hp); buffer.writeInt(vp); buffer.writeFloat(sb) }
        fun read(buffer: WireBuffer, ops: MutableList<Operation>) {
            ops.add(FitBoxLayout(null, buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readFloat())) }
    }
}
